...
export default function UsuariosCrud() {
    ...
    const usuariosFiltrados = useMemo(() => {
        return usuarios.filter((user) => {
            ...
            // 👇 CORRECCIÓN ERROR 7: Evitar "undefined.toString()" asegurando que sea un String seguro
            const claveSegura = String(user.clave_trabajador || '').toLowerCase();
            
            const coincideTexto = 
                nombreCompleto.includes(busqueda) || 
                user.email?.toLowerCase().includes(busqueda) ||
                claveSegura.includes(busqueda);
    ...
    const getRoleBadgeColor = (rol: string) => {
        switch (rol) {
            ...
            case 'Fabricacion': return 'bg-blue-100 text-blue-800 dark:bg-blue-500/20 dark:text-blue-300';
    ...
    return (
        <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6" >
                <div>
                    <h2 className="text-2xl font-bold text-slate-800 dark:text-white capitalize">Usuarios</h2>
                ...
                <div className="flex flex-col sm:flex-row gap-4 justify-between bg-white dark:bg-slate-950 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
                    <div className="relative max-w-md w-full">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 transition-colors text-slate-900 dark:text-white"
                        />
                    </div>
                    <button 
                        onClick={() => setMostrarFiltros(!mostrarFiltros)}
                        className={`w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border transition-colors ${mostrarFiltros ? 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-900/30 dark:text-indigo-400 dark:border-indigo-800' : 'bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700'}`}
                    >
                    ...
                    <div className="flex flex-col sm:flex-row gap-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800 animate-in fade-in slide-in-from-top-2">
                        <div className="w-full sm:w-auto">
                            ...
                            <select 
                                ...
                                className="w-full sm:w-48 px-3 py-2 bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 outline-none text-slate-700 dark:text-slate-200 transition-colors"
                            >
                                    ...
                                    usuariosFiltrados.map((user) => (
                                        <tr key={user.id_user} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <td className="px-6 py-4 text-center">
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${user.estatus ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300' : 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300'}`}>
