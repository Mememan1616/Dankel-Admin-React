...
import type { Producto } from '../../../interfaces/productos'; // Ajusta la ruta a tu interfaz
...
import {
    ...
    Search,
    Filter,
    Box, // Ícono para Producto
    Plus,
} from 'lucide-react';
import FormularioProducto from './formularioProducto'; // Asegúrate de crear/importar este componente
...
export default function ProductosCrud() {
    ...
    const getAllProductos = async () => {
        ...
        const productosData: Producto[] = await ApiService.getAllProductos();
        setProductos(productosData);
    };

    // --- FUNCIÓN PARA ABRIR EL FORMULARIO ---
    const MostrarFormulario = (action: string, producto?: Producto) => {
        setTitle(action + " producto");
        setIsModalOpen(true);
        setAction(action);
    ...
    const productosFiltrados = useMemo(() => {
        return productos.filter((prod) => {
            // 1. Búsqueda por texto (nombre del producto o descripción)
            const busqueda = searchTerm.toLowerCase();
            const coincideTexto = 
                prod.producto?.toLowerCase().includes(busqueda) || 
                prod.descripcion?.toLowerCase().includes(busqueda);

            // 2. Filtro por Estatus
            const coincideEstatus = 
                filtroEstatus === 'todos' ? true :
                filtroEstatus === 'activos' ? prod.estatus === true :
                prod.estatus === false;

            return coincideTexto && coincideEstatus;
        });
    }, [productos, searchTerm, filtroEstatus]);
    ...
    return (
        <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6" >
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                        Administra y visualiza el catálogo de productos.
                    </p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-950 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 transition-colors duration-300 overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse whitespace-nowrap min-w-full">
                            <thead>
                                <tr>
                                    <th className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">Producto</th>
                                    <th className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">Descripción</th>
                                    <th className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">Unidad de Medida</th>
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
                                {productosFiltrados.length > 0 ? (
                                    productosFiltrados.map((prod) => {
                                        return (
                                            <tr key={prod.id_producto} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        ...
                                                        <div>
                                                            <p className="text-sm font-medium text-slate-900 dark:text-white">{prod.producto}</p>
                                                            <p className="text-xs text-slate-500 dark:text-slate-400">ID: {prod.id_producto}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <p className="text-sm text-slate-600 dark:text-slate-400 max-w-[250px] sm:max-w-xs truncate" title={prod.descripcion}>
                                                        {prod.descripcion}
                                                    </p>
                                                </td>
                                                <td className="px-6 py-4">
                                                    {/* Ajusta "Sí/No" según lo que signifique tu booleano en negocio */}
                                                    <span className="text-sm text-slate-700 dark:text-slate-300 font-medium">
                                                        {prod.unidad_medida}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${prod.estatus
            ...
            <FormularioProducto
                ...
                action={action}
                producto={selectedProducto ? selectedProducto : undefined}
            />
        </>
    );
}