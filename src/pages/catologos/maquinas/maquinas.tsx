...
import FormularioMaquina from './formularioMaquina'; // Asegúrate de que la ruta sea correcta
...
export default function MaquinasCrud() {
    ...
    const getAllMaquinas = async () => {
        const maquinasData: Maquina[] = await ApiService.getAllMaquinas();
    ...
    const maquinasFiltradas = useMemo(() => {
        return maquinas.filter((maquina) => {
            // 1. Búsqueda por texto (nombre de la máquina o línea)
    ...
    return (
        <>
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6" >
                <div>
                    ...
                    <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">
                        Administra y visualiza los registros de Máquinas.
                    </p>
                </div>
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">Descripción</th>
                                     <th className="px-6 py-4 font-medium text-slate-700 dark:text-slate-200">Produccion por Hora</th>
                                                ...
                                                <td className="px-6 py-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className="p-2 rounded-lg bg-red-50 dark:bg-red-500/10 text-red-500 shrink-0">
                                                            <IconoMaquina className="w-5 h-5" />
                                                        </div>
                                                        <div>
                                                            <p className="text-sm font-medium text-slate-900 dark:text-white">{maquina.maquina}</p>
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <p className="text-sm text-slate-700 dark:text-slate-300 capitalize">{maquina.linea}</p>
                                                    <p className="text-xs text-slate-500 dark:text-slate-400">{maquina.id_linea}</p>
                                                </td>
                                                <td className="px-6 py-4">
                                                    <p className="text-sm text-slate-600 dark:text-slate-400 max-w-[200px] sm:max-w-xs truncate" title={maquina.descripcion}>
                                                        {maquina.descripcion}
                                                    </p>
                                                </td>
                                                 <td className="px-6 py-4">
                                                    <p className="text-sm text-slate-600 dark:text-slate-400 max-w-[200px] sm:max-w-xs truncate" title={maquina.descripcion}>
                                                        {maquina.produccionxHora}
                                                    </p>
                                                </td>
                                                <td className="px-6 py-4 text-center">
                                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${maquina.estatus
                                                        ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-500/20 dark:text-emerald-300'
                                                        : 'bg-slate-100 text-slate-800 dark:bg-slate-700 dark:text-slate-300'
                                                        }`}>
                                                        {maquina.estatus ? 'Activo' : 'Inactivo'}
                                                    </span>
                                                </td>
                                                <td className="px-6 py-4 text-right">
                                                    <div className="flex items-center justify-end gap-2">
                                                        ...
                                                        <button 
                                                            ...
                                                            title="Eliminar"
                                                            onClick={() => MostrarFormulario('Eliminar', maquina)}
                                                        >
            ...
            <FormularioMaquina
                ...
                title={title}
                action={action}
                maquina={selectedMaquina ? selectedMaquina : undefined}
                refreshData={getAllMaquinas}
